...
增量步骤(GATES/autonomy/可审计清单)是 qf 增量层入口,机制权威在各自文件、这里只串流程。 -->
...
3. **定门禁**:改动有风险(碰钱/信用/权限/不可逆动作/造正式事实/改 Agent 等)→ 先去 `dev/GATES.md` 按风险阶梯 R0–R5 定 **风险级 + 必备工件集(用 `tasks/_templates/` 模板)+ 必跑测试**;不打风险标不可进实现。
...
6. 红线见 `RULES.md`(通用,含**双轴门禁边界**) + `RULES.project.md`(本项目);**不可逆真钱/真信用/生产 flag/真 KMS 门永不降——永远人审双签,AI 不自动执行**;不擅自 commit;致命错误即停工。遇 `DECISIONS.md` 没覆盖的新岔路、**或 BOARD/卡标注的前置闸门(若有)**,停下问用户(无人值守按 `LOOP_CONTRACT` question-budget 处理)。

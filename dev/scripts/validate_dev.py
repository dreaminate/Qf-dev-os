...
**空骨架必须跑绿**：占位行用 `<...>` 标，沿用既有 `"<" not in status` 逻辑跳过。
...
def run_os_checks(dev: Path) -> tuple[list[str], list[str]]:
    ...
    required = [
        ...
        "ISSUES.md", "experience.md", "CODEMAP.md", "README.md",
...
def _canary_os(dev: Path) -> list[str]:
    ...
    for rel, (tokens, msg) in _OS_CANARY.items():
        p = dev / rel
        if not p.is_file():
            continue  # 文件缺已在 2c FAIL,此处只查"在但被掏空"
        txt = p.read_text(encoding="utf-8")
...
fails += t_fails
warns = _canary_rules(DEV) + _canary_os(DEV) + _lint_review_status(DEV) + t_warns

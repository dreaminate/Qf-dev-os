...
内容是「它解决什么 + 四台+5层结构 + 怎么 drop-in + memory seed + validator」;
项目状态/红线不写这里(那些进目标项目的 dev/)。【开发os级别】随骨架走。 -->

# Qf-dev-os

从 qf（群峰）实战里蒸馏出来的可复用开发操作系统：四台底座 + Goal Loop + 5 个严谨增量层，给「AI 驱动 / 长周期 / 无人值守 / 有真钱不可逆动作 / 多并发 agent」的开发用。直接搬进任何 git 仓库。

一个会写代码的 agent，在长周期、常常无人值守的状态下推进一个有真钱、有不可逆动作的产品，而且多个会话 / 并发 loop 同时在一个仓库上跑——这个处境逼出四个失效模式：**无法人工全审**（改动量大、夜里没人盯，每个 PR 人工签字物理上做不到）、**不可逆动作出事**（agent 自动执行了真钱 / 生产发布 / 不可逆删除，收不回）、**过度声明**（把「声称」当「已验证」，把单场景小样本叫「已证明」，把上轮数据当本轮结果）、**文档与状态漂移**（状态活在会被压缩的对话里、双源各写一份、并发会话互相 clobber，跑两天就没人知道真实进度）。

总答案一句话：**让严谨成比例 + 可审计 + 诚实标界 + 有界自治，人只守不可逆门。** 拿到改动先机械定级，低风险轻档放行、高风险才上全套工件；工程过程门可以不再强制人工签字，但交付物必须自带可审计清单；每条状态强制带「本条不证明什么 + 证据强度」，🟡声称 ≠ ✅验证；无人值守时每一轮是「读 live 状态 → 做一个有界增量 → 回写 / 卡住记账」，反划水、绝不「干到全 done」；而真钱 / 真信用 / 不可逆动作**永远人审双签**，这条门和工程门正交、永不降。关键洞察：**accountability 按「不可逆性」分轴，不按「能力」分轴**——agent 能不能自己起草实现可以放开，不可逆真钱动作该不该让 agent 自动执行永远不放开。

具体设计与取舍见 [`WHITEPAPER.md`](WHITEPAPER.md)，机制细节见 `dev/` 各权威文件。它是 **Dev-os 的严格超集**：只想要轻骨架 → 用纯 [Dev-os](https://github.com/dreaminate/Dev-os)；要严谨验收 + 风险门 + 无人值守 → 用本 OS（见末尾「相关仓库」）。
...
├── WHITEPAPER.md                      ← 「为什么这么设计」导读(qf 为活样本)        [框架·勿改]
...
└── dev/                               ← 整套开发 OS（drop-in 这一层）
    ...
    │
    ├─❷ 规约 / 红线
    │   ├── RULES.md                 OS 铁律宪法 §0–§10(含双轴门禁边界[权威])      [框架·勿改]
...
└────────────────────────────────────────────────────────────────────┘
 ...
 ② 正式事实 + 双轴门禁边界 ──► dev/RULES.md §8 + GATES §6   进 RULES 宪法
      工程门可降为「可审计」;不可逆真钱门永不降·永远人审双签
...
每个改动(PR / 任务 / 一回合实现)进实现前走三步,权威在 dev/GATES.md §1:
  ...
  ③ 打标+自述 ──► header 标 risk_level=R?,交付出可审计清单(§6)
   ...
   │ R4 │ 碰不可逆/资金/终决   │ +ADR +回滚/补偿 +release   │ R3 + 回滚演练       │
   │ R5 │ 系统级治理/生产学习  │ +TestEvidence +冻结方案     │ R4 + 多视角评审建议 │
   └────┴──────────────────┴───────────────────────┴──────────────────┘
...
### 四、双轴门禁边界（全 OS 最值钱的一刀）

```
两条正交的门,任何改动先分清在哪条轴上,别混。权威在 dev/RULES.md §8。

  ┌──────────────────────────┐        ┌──────────────────────────────┐
  │   轴 A:工程过程门          │        │   轴 B:运行时不可逆门          │
  │   评审 / 测试 / 文档同步     │        │   真钱 / 真信用 / 不可逆动作      │
  │                          │   ⊥    │   真争议裁决 / 生产 flag /        │
  │                          │ 正交    │   真 KMS / 对外发布              │
  ├──────────────────────────┤        ├──────────────────────────────┤
  │ 可降级为「可审计交付」       │        │ 永不降                         │
  │ 不再强制人工签字,改以        │        │ 永远人审(双签)                 │
  │ 可审计清单为准              │        │ AI 不得自动执行                 │
  │ (降级 ≠ 取消:可审计是硬要求) │        │ (工程去门禁不波及这条)           │
  └──────────────────────────┘        └──────────────────────────────┘
        让你敢把工程门降级换 throughput          又不会因此把不可逆真钱动作交给 agent
        (夜里无人值守也能推进)                   可审计取代的是评审签字,不是人类双签

  总触发器 = 正式业务事实四性「可证 / 可测 / 可逆 / 可审」:属于这个类的事实绝不能被
  伪造或丢失。四性 project-agnostic;「哪些具体动作落在不可逆门」由项目在 RULES.project §致命错误 枚举。
```
...
增量   5 层落地物存在:GATES + HARNESS_INDEX + 8 模板 + autonomy/* + runner + GAP_LOG(缺即 FAIL)
...
> **别复制本仓库根的 `README.md`**(它是 OS 安装说明,不是项目文件);`WHITEPAPER.md` 可选带(它是「为什么」导读)。
...
> **项目状态全在仓库 `dev/`**（新 session 读根 `CLAUDE.md` → dev/ 四台 + GATES + autonomy）。
> 本 memory 只装 dev/ 装不下的那层:操作者画像 / 工作偏好 / 外部参考凭据。
> 铁律:① 别复制 dev/ 项目状态(双源必漂),引用按**章节名**钉 ② 某条成熟成项目规则→升级进 dev/,不留第二份。

**操作者 / 怎么协作（user）**
- (按需:谁在操作、中/英、怎么和 agent 协作、**谁动真钱/谁拍不可逆门**)

**工作偏好（feedback）**
- (按需:commit 习惯、协作节奏、复核口味、无人值守节奏等)
...
- 碰**真钱 / 不可逆动作**:留人审(双签)入口,AI 不自动执行(`RULES.md §8`)。
...
- **[Dev-os](https://github.com/dreaminate/Dev-os)** — 底座与标尺。单写者轻骨架:强制读 + 可自检 + 防漂,四台 + Goal Loop + 诚实 gap。项目**不碰真钱 / 不可逆动作、不跑无人值守、单人单线**开发 → 用它,别背 5 层的重量。
- **Qf-dev-os（本仓库）** — **Dev-os 的严格超集**。在底座上叠 qf 实战提炼的 5 个增量层(风险门禁 / 双轴边界 / 工件证据 / 诚实纪律 / 自治多智能体)。项目**有真钱 / 不可逆动作、需严谨验收、跑无人值守 / 长 loop、被过度声明坑过** → 用它。
...
一句话选型:**单人轻量 → Dev-os;单写者但要严谨 + 无人值守 + 真钱门 → Qf-dev-os;多人协作 → Multi-Dev-Os。**

---

> **为什么这么装?** 这里只教你装。每层在挡哪个失效模式、qf 是怎么从一团活范式被蒸馏进来的、以及怎么照着提炼你自己项目的范式(7 步方法论)——全在 [`WHITEPAPER.md`](WHITEPAPER.md)。

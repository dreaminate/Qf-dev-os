...
不确定某事归哪个文件 → 先查 dev/HARNESS_INDEX.md(主索引;索引只导航,冲突以原文为准)

## 动手前(qf 增量层路由)
本 OS 在 Dev-os 四台之上多 5 个增量层——按需展开,别默认全读:
- **有风险的改动**(碰钱/信用/权限/不可逆动作/造正式事实/改 Agent 等)→ **先去 dev/GATES.md**(权威 grader)按风险阶梯 R0–R5 定**风险级 + 必备工件集 + 必跑测试**;不打风险标不可合。
- **无人值守 / 长跑 loop** → 先读 dev/autonomy/ 运行契约:LOOP_CONTRACT(循环边界 + 反划水)/ SAFETY_ENVELOPE(安全外壳)/ CONCURRENCY(并发纪律)/ ADVISOR_PROTOCOL(多模型顾问)。
- **若 dev/autonomy/OWNER_NEXT.md 存在 → 每次开工先读它**(owner 明文转向,压过一切自设目标);其次看 DECISION_RADAR.md 的待拍岔路。
- **诚实/非声明**:结论一律带 provenance + 证据强度(实证 / 🟡未验证 / HYPOTHESIS),小样本不说「证明/可信」——细则在 dev/RULES.md §诚实纪律 + dev/STATE.md 非声明列。

## 怎么干活(Goal Loop)
取 BOARD todo(**下一步以 BOARD 实时为准,STATE 头摘要可能滞后**;取卡先查 `review_status` + Open Questions 计数:**进实现须 review_status=1 且 待拍=0 两闸皆过;任一不满足(未过目 / 有待拍)→ 先停下请用户过目·拍板,不进实现**)
